...
test.describe('BrowserConsole Component', () => {
  ...
  test.describe('Component Rendering', () => {
    test('should render the console component', async ({ page }) => {
      const consoleEl = page.locator('browser-console')
      await expect(consoleEl).toBeVisible()
    })

    test('should have shadow DOM', async ({ page }) => {
      const hasShadow = await page.evaluate(() => {
        const el = document.querySelector('browser-console')
    ...
    test('should have filter buttons', async ({ page }) => {
      await expect(page.locator('browser-console').getByRole('button', { name: 'All', exact: true })).toBeVisible()
      await expect(page.locator('browser-console').getByRole('button', { name: 'Log', exact: true })).toBeVisible()
      await expect(page.locator('browser-console').getByRole('button', { name: 'Info', exact: true })).toBeVisible()
      await expect(page.locator('browser-console').getByRole('button', { name: 'Warn', exact: true })).toBeVisible()
    ...
    test('should have theme and clear buttons', async ({ page }) => {
      await expect(page.locator('browser-console').getByRole('button', { name: 'Toggle light/dark theme' })).toBeVisible()
      await expect(page.locator('browser-console').getByRole('button', { name: 'Clear all logs' })).toBeVisible()
    })
  })
  ...
  test.describe('Console Logging', () => {
    test('should capture console.log', async ({ page }) => {
      ...
      const logEntry = page.locator('browser-console').locator('[role="log"]').getByText('[LOG]')
      await expect(logEntry).toBeVisible()
    })

    test('should capture console.info', async ({ page }) => {
      await page.getByRole('button', { name: 'console.info()' }).click()
      const logEntry = page.locator('browser-console').locator('[role="log"]').getByText('[INFO]')
      await expect(logEntry).toBeVisible()
    })

    test('should capture console.warn', async ({ page }) => {
      await page.getByRole('button', { name: 'console.warn()' }).click()
      const logEntry = page.locator('browser-console').locator('[role="log"]').getByText('[WARN]')
      await expect(logEntry).toBeVisible()
    })

    test('should capture console.error', async ({ page }) => {
      await page.getByRole('button', { name: 'console.error()' }).click()
      const logEntry = page.locator('browser-console').locator('[role="log"]').getByText('[ERROR]')
      await expect(logEntry).toBeVisible()
    })

    test('should display objects with expandable view', async ({ page }) => {
      await page.getByRole('button', { name: 'Object', exact: true }).click()
      const expandable = page.locator('browser-console').locator('[role="log"]').getByRole('button', { name: /Object.*name/ })
  ...
  test.describe('Expand/Collapse', () => {
    test('should expand object when clicked', async ({ page }) => {
      await page.locator('browser-console').getByRole('button', { name: 'Clear all logs' }).click()
      await page.getByRole('button', { name: 'Object', exact: true }).click()

      const expandable = page.locator('browser-console').locator('[role="log"]').getByRole('button', { name: /Object.*name/ }).first()
      ...
      const properties = page.locator('browser-console').locator('[role="region"]').filter({ hasText: 'name:' }).first()
  ...
  test.describe('Filtering', () => {
    test('should filter by log type', async ({ page }) => {
      ...
      await expect(page.locator('browser-console').locator('[role="log"]').getByText('[ERROR]').first()).toBeVisible()
      await expect(page.locator('browser-console').locator('[role="log"]').getByText('[LOG]')).not.toBeVisible()
    })
    ...
    test('should show all logs when All filter selected', async ({ page }) => {
      ...
      await page.locator('browser-console').getByRole('button', { name: 'Error', exact: true }).click()
      await page.locator('browser-console').getByRole('button', { name: 'All', exact: true }).click()

      await expect(page.locator('browser-console').locator('[role="log"]').getByText('[LOG]').first()).toBeVisible()
      await expect(page.locator('browser-console').locator('[role="log"]').getByText('[ERROR]').first()).toBeVisible()
    })

    test('should update aria-pressed on filter buttons', async ({ page }) => {
      const allBtn = page.locator('browser-console').getByRole('button', { name: 'All', exact: true })
  ...
  test.describe('Theme Toggle', () => {
    test('should toggle between dark and light themes', async ({ page }) => {
      const themeBtn = page.locator('browser-console').getByRole('button', { name: /toggle.*theme/i })
      const consoleFeed = page.locator('browser-console').locator('.console-feed')
  ...
  test.describe('Data Type Formatting', () => {
    test('should format arrays', async ({ page }) => {
      ...
      const logEntry = page.locator('browser-console').locator('[role="log"]').getByText(/Array\(\d+\)/).first()
      await expect(logEntry).toBeVisible()
    })
    ...
    test('should format console.table', async ({ page }) => {
      ...
      const tableMethod = page.locator('browser-console').locator('[role="log"]').getByText('[TABLE]').first()
      await expect(tableMethod).toBeVisible()

      const table = page.locator('browser-console').locator('[role="log"] table').first()
    ...
    test('should format dates', async ({ page }) => {
      ...
      const dateEntry = page.locator('browser-console').locator('[role="log"]').getByText(/\d{4}-\d{2}-\d{2}T/).first()
  ...
  test.describe('Map and Set Support', () => {
    test('should format Map correctly', async ({ page }) => {
      ...
      const mapEntry = page.locator('browser-console').locator('[role="log"]').getByText(/Map\(2\)/).first()
    ...
    test('should format Set correctly', async ({ page }) => {
      ...
      const setEntry = page.locator('browser-console').locator('[role="log"]').getByText(/Set\(4\)/).first()
  ...
  test.describe('Symbol Support', () => {
    test('should display Symbol keys in objects', async ({ page }) => {
      ...
      const symbolEntry = page.locator('browser-console').locator('[role="log"]').getByText(/Symbol\(secret\)/).first()
  ...
  test.describe('Accessibility', () => {
    test('should have proper ARIA landmarks', async ({ page }) => {
      await expect(page.locator('browser-console').getByRole('region', { name: 'Console output' })).toBeVisible()
      await expect(page.locator('browser-console').getByRole('toolbar', { name: 'Console controls' })).toBeVisible()
      await expect(page.locator('browser-console').getByRole('log', { name: 'Console log entries' })).toBeVisible()
    })

    test('should have proper button groups', async ({ page }) => {
      await expect(page.locator('browser-console').getByRole('group', { name: 'Filter by log type' })).toBeVisible()
      await expect(page.locator('browser-console').getByRole('group', { name: 'Console actions' })).toBeVisible()
    })
